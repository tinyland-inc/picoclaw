...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in TinyClaw.
...
10. [Creating a New Provider in TinyClaw](#creating-a-new-provider-in-tinyclaw)
...
const antigravityPlugin = {
  ...
  register(api: TinyClawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<TinyClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- TinyClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.tinyclaw/auth.json`:
...
## Creating a New Provider in TinyClaw

TinyClaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/tinyclaw/cmd_auth.go`:
...
tinyclaw auth login --provider your-provider

# List models (for Antigravity)
tinyclaw auth models

# Start the gateway
tinyclaw gateway

# Run an agent with a specific model
tinyclaw agent -m "Hello" --model your-model
```
...
export TINYCLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export TINYCLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `tinyclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.tinyclaw/auth.json`
- Re-run `tinyclaw auth login --provider antigravity`
