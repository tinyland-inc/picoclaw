(**
  TinyClaw.Tailscale -- Verified Tailscale client specification.
...
*)
module TinyClaw.Tailscale

open FStar.List.Tot
open TinyClaw.Network
...
(** Default Setec namespace for TinyClaw *)
let tinyclaw_secret_prefix : string = "tinyclaw"

(** Well-known secret names *)
let secret_api_key (provider: string) : string =
  build_secret_name tinyclaw_secret_prefix (String.concat "/" ["providers"; provider; "api-key"])

let secret_webhook_key (platform: string) : string =
  build_secret_name tinyclaw_secret_prefix (String.concat "/" ["webhooks"; platform; "key"])

(** Proof: secret names are prefixed *)
let secret_has_prefix (name: string)
  : Lemma (String.length (build_secret_name tinyclaw_secret_prefix name) >
           String.length tinyclaw_secret_prefix)
  = ()
