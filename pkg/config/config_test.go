...
func TestLoadDhallConfig_NilWhenNoBinary(t *testing.T) {
	// LoadDhallConfig returns ErrDhallNotAvailable if dhall-to-json is not on PATH
	...
	if errors.Is(err, ErrDhallNotAvailable) {
		t.Log("dhall-to-json not found, LoadDhallConfig returned ErrDhallNotAvailable (correct)")
	} else if err != nil {
